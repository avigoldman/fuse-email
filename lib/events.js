...
function EventsContext() {
  ...
  context.on = function(event, listener) {
    var events = context.toArray(event);
  ...
  context.hears = function(pattern, events, listener) {
    var patterns = context.toArray(pattern);
    ...
    return context.on(events, function() {
      if (self.logger)
        self.logger.verbose('checking hears pattern');

      if (self.hearTest(patterns, arguments)) {
        if (self.logger)
          self.logger.verbose('hears pattern matched');
      ...
      if (self.logger)
        self.logger.verbose('pattern did not match');
    });
  };

  context._defaultHearTest = function(patterns, params) {
    var testString = _.toString(params[0]);
