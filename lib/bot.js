...
function EmailBot(config) {
  var sparky = {
    events: {},
    convos: [],
    email: null,
    config: _.defaults(config || {}, configDefaults, {
      htmlToTextOpts: htmlToTextOpts,
  ...
  sparky.processMessage = function(data) {
    ...
    if (_.toLower(_.trim(data.rcpt_to)) !== cleanInboundAddress && sparky.config.restrict_inbound === true) {
      logger.debug('Message sent to unknown address: '+data.rcpt_to);
      return;
    }
  ...
  sparky.bot = function(receivedMessage) {
    var bot = {
      ...
      startConversation: function(config, cb) {
        ...
        config.recipients = config.recipients || sparky.email.getRecipientList(receivedMessage, 'to');
        config.cc = config.cc || sparky.email.getRecipientList(receivedMessage, 'cc');
  ...
  sparky.hears = function(pattern, events, cb) {
    ...
    sparky.on(events, function(bot, message) {
      if (sparky.didIHear(patterns, message)) {
  ...
  sparky.didIHear = function(patterns, message) {
    let subjectPatterns = []
      , bodyPatterns =  [];
    ...
    _.each(bodyPatterns, (pattern) => {
      ...
      if (match = parseReply(sparky.clean(message.html)).match(test)) {
        matches.push({
          source: 'html',
          match: match
        });
      ...
      if (match = parseReply(message.text).match(test)) {
        matches.push({
          source: 'text',
          match: match
        });
  ...
  sparky.clean = function(input) {
    return htmlToText.fromString(input, sparky.config.htmlToTextOpts).replace(/(\r\n|\n|\r)/gm, "").trim();
  };

  sparky.getLatest = function(message) {
