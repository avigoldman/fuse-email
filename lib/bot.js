...
function Sparky(config) {
  ...
  sparky.processMessage = function(data) {
    ...
    let clean_text            = data.content.text.toLowerCase();
    ...
    // direct_mention
    if (clean_text.indexOf(cleanSendingDomain) >= 0
     || clean_text.indexOf(cleanInboundDomain) >= 0) {
      logger.debug('Trigger "direct_mention" event');
      sparky.trigger('direct_mention', triggerParams);
    }

    // mention
    if (clean_text.indexOf(sparky.config.bot_name.toLowerCase()) >= 0) {
