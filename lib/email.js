...
function Email(config) {
  ...
  email.send = function(message, cb) {
    ...
    if (template_id === null) {
      content = {
        from: message.from || '"' + email.config.bot_name + '" <' + email.config.sending_address + '>',
        reply_to: message.reply_to || '"' + email.config.bot_name + '" <' + email.config.inbound_address + '>',
        subject: message.subject || '',
        html: message.html || '',
        text: message.text || htmlToText.fromString(message.html || '', config.htmlToTextOpts),
